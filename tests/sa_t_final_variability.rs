...
#[test]
#[ignore = "SA t_final variability: long-running, run explicitly"]
fn test_sa_t_final_variability_per_instance() -> Result<(), Box<dyn std::error::Error>> {
    println!("Loading VRP instances...");
    let files = get_vrp_files()?;
    fs::create_dir_all("plots")?;

    // Values for final temperature to evaluate
    let t_finals = vec![0.001, 0.01, 0.05, 0.1, 0.5, 1.0, 5.0, 10.0];
    ...
    for file in files {
        ...
        let mut csv = fs::File::create(format!("plots/sa_t_final_{}.csv", fname))?;
        writeln!(
            csv,
            "t_final,mean_distance,std_distance,feasible_rate,mean_elapsed_ms,mean_iterations"
        )?;
        ...
        for &t_final in &t_finals {
            ...
            writeln!(
                csv,
                "{:.6},{:.6},{:.6},{:.3},{:.3},{:.1}",
                t_final, mean_d, std_d, feasible_rate, mean_t, mean_iter
            )?;

            series_points.push((t_final, mean_d));
        }

        let series = vec![("mean_distance", series_points.clone())];
        let plot_path = format!("plots/sa_t_final_{}.png", fname);
        plot_multi_series_line(
            &plot_path,
            &format!("SA: t_final sensitivity — {}", fname),
            "t_final",
            "mean distance",
            &series,
        )?;

        println!("  Saved plots/sa_t_final_{}.png and .csv", fname);
    }

    println!("SA t_final variability analysis complete. CSVs and PNGs in plots/");
    Ok(())
}
